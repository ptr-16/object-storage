...
def get_object(bucket_name, object_name):
    ...
    try:
        ...
        print(f"Data from '{object_name}': {response.read().decode('utf-8')}")
    except S3Error as exc:
        print(f"Error retrieving object: {exc}")
def delete_object(bucket_name, object_name):
    """Delete an object from the specified bucket."""
    try:
        client_minio.remove_object(bucket_name, object_name)
        print(f"'{object_name}' removed successfully from '{bucket_name}'.")
    except S3Error as exc:
        print(f"Error removing object: {exc}")
def update_object(bucket_name, object_name):
    ...
    try:
        ...
        print(f"'{object_name}' updated successfully in '{bucket_name}'.")
        ...
        print(f"Error updating object: {exc}")
